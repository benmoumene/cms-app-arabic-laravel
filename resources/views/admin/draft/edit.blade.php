...
<div class="main_container  main_menu_open">
                ...
                <div class="page_content">
                   ...
                   <div class="form">
                        <form class="form-horizontal" enctype="multipart/form-data" action="{{route('admin.draft.update',$draft->id)}}" method="post" >
                                    @csrf                    
                      @method('PUT')
                                <div class="form-group">
                                    <label for="ad_title">عنوان اشاري</label>
                                    <input type="text" name="reference_address" value="{{$draft->reference_address}}" class="form-control" id="ad_title" placeholder="اسم او عنوان الاعلان">
                                </div>
                                <div class="form-group">
                                    <label for="ad_title">عنوان الخبر</label>
                                    <input type="text" name="drafts_address" value="{{$draft->news_address}}" class="form-control" id="ad_title" placeholder="اسم او عنوان الاعلان">
                                </div>
                                <div class="form-group">
                                    <label for="ad_title">المصدر</label>
                                    <input type="text" name="source" value="{{$draft->source}}" class="form-control" id="ad_title" placeholder="اسم او عنوان الاعلان">
                                </div>
                                
                                <div class="form-group">
                                    <label for="ad_img">* صوره الخبر</label>
                                    <input type="file"  id="imgInp" name="image" value="{{$draft->image}}" class="form-control" id="ad_img">
                                    <img  id="blah"  src="{{ asset('images/draft/'.$draft->image) }}" width="150">
                            ...
                            <div class="form-group">
                                <label for="exampleFormControlSelect1">الكاتب</label>
                                    <select class="form-control" name="new_writer"  id="draft_writer" >
                                        @foreach($users as $user)
                                        <option  value="{{ $user->name }}" @if($draft->new_writer=='$user->name')selected @endif > {{ $user->name }}</option>
                                        @endforeach
                                    ...
                                    <textarea id="edit" name="new_body">{!!  $draft->new_body !!}.</textarea>
                                    </div>
                                    </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="link" value="{{$draft->link}}" name="link" class="form-control" id="ad_link" placeholder="EX : http://www.google.com">
                                </div>
                                ...
                                <label for="exampleFormControlSelect1">التصنيفات</label>
                                    <select class="form-control" name="category_id"  id="category_id">
                                        ...
                                        <option  value="{{ $category->id }}"  @if($draft->category_id== $category->id)selected @endif> {{ $category->name }}</option>
                                        @endforeach
                                        
                                    </select>
                                </div>
                                <div class="form-group">
                                <input type="checkbox" name="position[]" value="1" @if(Str::contains($draft->position,"1")) checked  @endif  ><label>شريط الاخبار (ظهور عنوان الخبر في شريط المتحرك )</label>
                                <br>
                                <input type="checkbox" name="position[]" value="2"@if(Str::contains($draft->position,"2"))checked @endif  ><label> خبر رئيسي (في منطقة التميز ) </label>
                                <br>
                                <input type="checkbox" name="position[]" value="3" @if(Str::contains($draft->position,"3"))checked  @endif  >  <label> الظهور في القالب  </label> 
                                <br>
                                <input type="checkbox" name="position[]" value="4" @if(Str::contains($draft->position,"4")) checked  @endif > <label> خبر مخفي </label> 
