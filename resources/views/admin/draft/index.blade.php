...
<div class="row main_container_head" style="margin-top:20px;margin-right:-60px;" >
                    <h4 ><span class="glyphicon glyphicon-folder-open"></span> المسودة </h4>
                </div>
       ...
       <!-- <div class="job-field">
            ...
            -->
                    ...
                    <div class="admin_index" style="margin-top:40px;">
                        ...
                        <div class="panel-heading text-right h4">عرض مسودة </div>
                            <table class="table" id="myList" >
                                ...
                                <tr class="text-center" id="draft-{{$draft->id}}">
                                    <td class="english">{{ $draft->id}}</td>
                                    <td> {{ $draft->reference_address}} </td>
                                    <td>{{$draft->updates_count}}</td>
                                    <td>0</td>
                                    <td class="english">{{ $draft->category->name}}</td>
                                    <td class="english">{{ $draft->new_writer}}</td>
                                    <td class="english">{{ $draft->created_at}}</td>
                                    <td class="text-center">
                                    <form>
                                        <a name="draft" href="{{route('admin.draft.edit',$draft->id)}}"  class="btn btn-warning edit" title="edit"><i class="glyphicon glyphicon-pencil"></i></a>
                                        <button type="submit" class="btn btn-danger delete"  data-id="{{$draft->id}}" title="delete" ><i class="glyphicon glyphicon-remove"></i></button>
                                    </form>
                                    </td>
                                </tr>
                                @endforeach
...
$(".delete").click(function(e){
            e.preventDefault();
              ...
              $.ajax(
              {
                  url: "/adminPanel/draft/delete/"+id,
                  type: 'DELETE',
                  dataType: "JSON",
                  data: {
                      "id": id,
